    ...
    if request.method!="POST":
        ...
        try:
            ...
            if medical_certificate:
                leave_report.medical_certificate = medical_certificate
            leave_report.save()
...
def save_subjects(request):
    if request.method == 'POST':
        ...
        try:
            # Clear existing subject selections for the current user
            Subject.objects.filter(student=request.user).delete()
            # Create new subject selections
            for subject_name in selected_subjects:
                subject = Subject.objects.create(name=subject_name, selected=True, student=request.user)
                subject.save()
            messages.success(request, "Exam Registration Success")  # Redirect to a success page
            return HttpResponseRedirect(reverse("student_apply_exam"))
        except Exception as e:
            messages.error(request, "Failed to save subjects: " + str(e))
            return HttpResponseRedirect(reverse("student_apply_exam"))  # Redirect to a failure page
    else:
        messages.error(request, "Invalid request method")
        return HttpResponseRedirect(reverse("student_apply_exam"))  # Redirect to a failure page

def apply_exam(request):
    return render(request, 'student_template/apply_exam.html')
